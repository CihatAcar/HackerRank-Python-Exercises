...
The characters in u are a subsequence of the characters in t_i.
Any repeat occurrence of a character is removed from the string such that each character in u, occurs exactly once.
In other words, if the character at some index j in t_i occurs at a previous index < j in t_i,
...
The first substring is all 'A' characters, so u_1 = A. The second substring has all distinct characters, so u_2 = BCA.
The third substring has different characters, so u_3 = DE.
...
def merge_the_tools(string, k):
    ...
    num_subsequences = len(string) // k
    for i in range(num_subsequences):
        substring = string[i * k: (i + 1) * k]
        distinct_chars = "".join(OrderedDict.fromkeys(substring))
